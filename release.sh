...
# EdDSA key, regenerates appcast.xml, tags the commit, and pushes a GitHub
# release. The new appcast.xml is committed to main so installed clients
...
# Refuse to release with a dirty tree or unpushed appcast — easy footgun.
...
# Regenerate appcast.xml. Sparkle accepts arbitrary additional <item> entries
...
cat > "${SCRIPT_DIR}/appcast.xml" <<EOF
