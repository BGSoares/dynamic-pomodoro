#!/usr/bin/env bash
#
# dynamic-pomodoro.sh — a macOS-native dynamic pomodoro loop in one script.
#
# Focus durations follow a cosine curve across the workday: short at the
# edges, peak around midday. Breaks come with a prescribed activity drawn
# from activities.json. The screen locks 30s into each break.
#
# Dependencies: bash, python3, jq, osascript, pmset (macOS).
#
# Usage:
#   ./dynamic-pomodoro.sh              # loop forever
#   ./dynamic-pomodoro.sh --once       # one focus+break cycle
#   ./dynamic-pomodoro.sh --dry-run    # print plan, don't sleep
#
# State + log live in:
#   ~/Library/Application Support/LightPomodoro/
...
# ---- paths ----
SUPPORT_DIR="$HOME/Library/Application Support/LightPomodoro"
STATE_FILE="$SUPPORT_DIR/state"
LOG_FILE="$SUPPORT_DIR/sessions.jsonl"
ACTIVITIES_JSON="$(cd "$(dirname "$0")" && pwd)/activities.json"
mkdir -p "$SUPPORT_DIR"
